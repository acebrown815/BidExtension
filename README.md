...
- **Auto-Bid (Beta)** — seed rows with just a Link (and optionally a Title), leaving Company/Location/Salary/ResumeNo blank to mark them pending. The **Auto-Bid** tab's **Analyze Pending Jobs** button opens up to 10 pending rows' Links in new background tabs — one after another, not all at once, to avoid a network/rate-limit burst — and runs Analyze Job on each automatically. A job that scores above 75% (the same bar Mark as Applied is already gated on) also gets its application form autofilled automatically on the spot, using whichever resume actually won the analysis. Nothing is ever submitted — reviewing the autofilled form and clicking Mark as Applied is still manual.
