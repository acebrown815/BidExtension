...
- **Auto-Bid (Beta)** — seed rows with just a Link (and optionally a Title), leaving Company/Location/Salary/ResumeNo blank to mark them pending. The **Auto-Bid** tab's **Analyze Pending Jobs** button opens up to 10 pending rows' Links in new background tabs — one after another, not all at once, to avoid a network/rate-limit burst — and runs Analyze Job on each automatically. A job that scores above 75% (the same bar Mark as Applied is already gated on) also gets its application form autofilled automatically on the spot, using whichever resume actually won the analysis. On job boards where the posting page has no form yet — just an "Apply Now" link to a separate application page (confirmed on CATS/catsone.com) — that link is clicked automatically first, and the resulting page is autofilled the same way. Nothing is ever submitted — reviewing the autofilled form and clicking Mark as Applied is still manual.
