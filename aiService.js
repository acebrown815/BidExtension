 ...
 * the extension. It currently supports a single provider:
 *   - OpenAI                 — OpenAI chat completions style
 *
 * The module was previously a multi-provider abstraction (Anthropic, Google
 * Gemini, Groq, Cerebras, Together AI, OpenRouter, Mistral AI, DeepSeek,
 * Cohere). Support for those providers was intentionally removed to keep
 * the extension scoped to a single, well-tested provider. The PROVIDERS
 * registry and apiStyle dispatch below are kept generic so another
 * provider can be re-added the same way in the future if needed.
 *
...
const PROVIDERS = {
  ...
  // ── OpenAI ────────────────────────────────────────────────────────
  // The canonical OpenAI Chat Completions API.
 ...
 *
 * @param {string} provider  - Key into PROVIDERS (currently only 'openai').
...
//
// Quirks: OpenAI's newer models (the o-series reasoning models, GPT-5 and
// later) impose stricter request shapes than older chat models:
//   1. They reject the legacy 'max_tokens' parameter with an HTTP 400 and
//      require 'max_completion_tokens' instead.
//   2. Some reject any non-default 'temperature' value ("Only the default
//      (1) value is supported") and require the field to be omitted
//      entirely.
// Rather than hardcode a model-ID allowlist that will always lag behind
// OpenAI's releases, fetchOpenAI detects these specific 400 responses and
// transparently adjusts the request, retrying until it either succeeds or
// runs out of known adjustments. The combination of quirks a model needs
// is remembered for the lifetime of this worker so later calls for the
// same model apply the fix on the first attempt instead of paying for a
// failed request every time.
//

/**
 * Per-model request-shape quirks discovered at runtime, keyed by model id.
 * @type {Map<string, {tokenParam: 'max_tokens'|'max_completion_tokens', omitTemperature: boolean}>}
 */
const openAIModelQuirks = new Map();

/** Maximum number of adjustment attempts before giving up and surfacing the last error. */
const MAX_QUIRK_ATTEMPTS = 3;

/**
 * Calls the OpenAI Chat Completions endpoint.
 *
 * @param {Object} config   - OpenAI provider config from PROVIDERS (apiStyle: 'openai').
 * @param {string} apiKey   - Bearer token for OpenAI.
...
async function fetchOpenAI(config, apiKey, messages, params) {
  ...
  const buildBody = (quirks) => {
    const body = {
      model: params.model,
      [quirks.tokenParam]: params.maxTokens,
      messages
    };
    if (!quirks.omitTemperature) body.temperature = params.temperature;
    return JSON.stringify(body);
  };

  const existing = openAIModelQuirks.get(params.model);
  const quirks = existing ? { ...existing } : { tokenParam: 'max_tokens', omitTemperature: false };

  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 60000);
  try {
    let resp, errorText;
    for (let attempt = 0; attempt < MAX_QUIRK_ATTEMPTS; attempt++) {
      resp = await fetch(config.endpoint, {
        method: 'POST',
        signal: controller.signal,
        headers,
        body: buildBody(quirks)
      });
      if (resp.ok) break;

      errorText = await resp.text();
      if (resp.status !== 400) throwAPIError(resp.status, errorText);

      // Inspect the 400 body for a known, fixable quirk and adjust the
      // request accordingly. If neither applies (or the fix was already
      // applied and it's still failing), surface the error as-is.
      let adjusted = false;
      if (quirks.tokenParam === 'max_tokens' && /max_completion_tokens/.test(errorText)) {
        quirks.tokenParam = 'max_completion_tokens';
        adjusted = true;
      }
      if (!quirks.omitTemperature && /temperature/i.test(errorText) && /does not support/i.test(errorText)) {
        quirks.omitTemperature = true;
        adjusted = true;
      }
      if (!adjusted) throwAPIError(resp.status, errorText);
    }

    if (!resp.ok) throwAPIError(resp.status, errorText);

    // Remember the working combination so future calls for this model
    // skip straight to a request shape that succeeds.
    openAIModelQuirks.set(params.model, quirks);
...
function buildSingleBulletRewritePrompt(originalBullet, jobDescription, missingSkills, currentEdit, excludedSkills) {
  ...
  const skillsGuidance = [
    missing.length > 0
      ? `- Weave in these missing skills wherever they can plausibly fit this bullet's work: ${missing.join(', ')}. If the original bullet names a technology/skill this job description has no real use for, REPLACE that mention with one of the missing skills above instead of just leaving both in — don't pad the bullet with irrelevant tech just because the original happened to mention it.`
      : '- No specific skills to target for this bullet — just improve it to better match the job description below',
    excluded.length > 0
