 ...
 *   - Multi-resume management: unlimited named resume profiles that can be
 *     added, deleted, switched, renamed, and persisted independently in
 *     chrome.storage.local, plus a per-resume ATS keywords summary derived
 *     from each resume's own skills/certifications/project technologies
...
async function autoSaveProfile() {
  ...
  try {
    ...
    ensureActiveResume().profile = JSON.parse(JSON.stringify(profileData));
 ...
 * Keyed by provider ID (currently only 'openai').  Used to populate the
...
/**
 * Activates a tab by name: deactivates all others, shows the matching
 * `.tab-content` panel (IDs follow the convention "tab-<name>"), and
 * lazy-loads data for tabs that fetch it on demand. Shared by the tab-click
 ...
 * Activates the clicked tab and updates the URL's #hash to match, so the
 * address bar always reflects whichever tab is visible (bookmarking,
 * copying the URL, or refreshing lands back on the same tab instead of
 * whatever hash the page happened to load with). Uses history.replaceState
 * rather than assigning location.hash directly so this doesn't also fire a
 * `hashchange` event — which would re-run handleHash() and call
 * activateTab() a second time for the same click — and doesn't add a
 * browser-history entry per click (tabs aren't separate pages; only the
 * page's initial deep-link and real back/forward navigation should move
 * the fragment).
 */
...
async function handleFile(file) {
  ...
  try {
    ...
    // If this is the very first resume ever uploaded, there's no active
    // resume yet to attach it to — create one automatically. Otherwise sync
    // the freshly-parsed profile into the currently active resume entry.
    ensureActiveResume().profile = JSON.parse(JSON.stringify(profileData));

    // Store raw DOCX bytes for tailored resume generation (direct DOCX editing).
    // Also mirrored into the active resume's own entry in `resumes` so each
    // resume keeps its own raw file instead of sharing a single global one.
    let rawBase64 = null;
    if (ext === 'docx') {
    ...
    await sendMessage({ type: 'SAVE_RAW_RESUME', rawResumeBase64: rawBase64, fileType: ext });
    const activeForRaw = getActiveResume();
    if (activeForRaw) {
      activeForRaw.rawResumeBase64 = rawBase64;
      activeForRaw.resumeFileType = ext;
    }
    await chrome.storage.local.set({ resumes, activeResumeId });
    renderResumeList();
 ...
 * 3. Deep-copies the updated profile into the active resume and writes
 *    `resumes` back to chrome.storage.local so resume state stays consistent.
 */
document.getElementById('saveProfileBtn').addEventListener('click', async () => {
  ...
  try {
    await sendMessage({ type: 'SAVE_PROFILE', profile: profileData });
    // Deep-copy into the active resume so `resumes` always reflects the latest save
    ensureActiveResume().profile = JSON.parse(JSON.stringify(profileData));
    await chrome.storage.local.set({ resumes, activeResumeId });
    renderResumeList();
    markProfileClean();
...
/**
 * "Export" button handler — downloads the current qaList as a JSON file so
 * it can be re-imported into JobMatch AI on another browser or computer
 * instead of retyping every answer.
 */
document.getElementById('exportQABtn').addEventListener('click', () => {
  if (!qaList.length) {
    showToast('No Q&A answers to export yet.');
    return;
  }
  ...
  a.href     = url;
  a.download = `jobmatch-ai-qa-answers-${dateStr}.json`;
  document.body.appendChild(a);
  a.click();
  a.remove();
  URL.revokeObjectURL(url);
  showToast(`Exported ${qaList.length} Q&A answer${qaList.length === 1 ? '' : 's'}.`);
});

/** "Import" button — just opens the hidden file picker. */
document.getElementById('importQABtn').addEventListener('click', () => {
  document.getElementById('qaImportFile').click();
});

/**
 * File picker change handler — reads the selected JSON export and merges it
 * into the current qaList by question text: an imported entry updates the
 * matching existing entry's answer/category/type/options, or is appended if
 * no match exists. Never removes an existing entry the import doesn't
 * mention, so importing on a browser that already has some answers filled
 * in is safe.
 */
document.getElementById('qaImportFile').addEventListener('change', async (e) => {
  ...
  try {
    ...
    showToast('Could not read that file — is it a valid JSON export?');
    return;
  }
  if (!Array.isArray(imported)) {
    showToast("That file doesn't look like a Q&A export (expected a JSON array).");
    return;
  }

  const byQuestion = new Map(qaList.map((q, i) => [q.question.toLowerCase().trim(), i]));
  let added = 0, updated = 0, skipped = 0;
  for (const raw of imported) {
    const question = String((raw && raw.question) || '').trim();
    if (!question) { skipped++; continue; }
    const entry = {
      question,
      answer: String((raw && raw.answer) || ''),
      category: (raw && raw.category) ? String(raw.category) : 'custom',
      ...((raw && raw.type) ? { type: raw.type } : {}),
      ...(Array.isArray(raw && raw.options) ? { options: raw.options } : {}),
    };
    const key = question.toLowerCase();
    if (byQuestion.has(key)) {
      qaList[byQuestion.get(key)] = entry;
      updated++;
    } else if (qaList.length >= 200) {
      skipped++;
    } else {
      qaList.push(entry);
      byQuestion.set(key, qaList.length - 1);
      added++;
    }
  }

  if (added === 0 && updated === 0) {
    showToast(skipped ? `Nothing importable — ${skipped} entr${skipped === 1 ? 'y was' : 'ies were'} skipped.` : 'Nothing to import.');
    return;
  }

  qaList = migrateQAList(qaList);
  renderQA();
  if (qaList.some(q => q.category)) {
    document.getElementById('qaCategoryFilter').style.display = 'block';
  }
  ...
  try {
    ...
    showToast(`Imported ${added} added, ${updated} updated — but failed to save: ${err.message}`);
    return;
  }
  const skipNote = skipped ? `, ${skipped} skipped` : '';
  showToast(`Imported and saved: ${added} added, ${updated} updated${skipNote}.`);
});

/**
 * "Export All Data" — downloads the user's whole JobMatch AI configuration
 * (profile, resumes, Q&A answers, saved/applied jobs, AI Settings, and
 * Google Sheets sync) as one JSON file, for moving to another browser or
 * computer via "Import All Data" there. Unlike the Q&A-only export above,
 * this covers everything on the Profile and Settings tabs in one file.
 */
document.getElementById('exportAllBtn').addEventListener('click', async () => {
  let payload;
  try {
    payload = await sendMessage({ type: 'EXPORT_ALL_DATA' });
  } catch (err) {
  ...
  const a    = document.createElement('a');
  a.href     = url;
  a.download = `jobmatch-ai-backup-${dateStr}.json`;
  document.body.appendChild(a);
  a.click();
  a.remove();
  URL.revokeObjectURL(url);
  showToast('Exported your JobMatch AI data. Keep the file private — it includes your API key and Sheets sync secret.');
});

/** "Import All Data" — just opens the hidden file picker. */
document.getElementById('importAllBtn').addEventListener('click', () => {
  document.getElementById('allDataImportFile').click();
});

/**
 * File picker change handler for the full backup import. Unlike the
 * Q&A-only import (which merges by question text), this OVERWRITES every
 * section present in the file — the whole point is "set this browser up to
 * match that one", so a partial merge would leave a confusing mix of old
 * and new profile/resumes/settings. Confirms with the user first since
 * it's destructive to whatever is already on this browser, then reloads
 * the page so every tab re-reads the restored data from storage instead of
 * needing each section's load logic duplicated here.
 */
document.getElementById('allDataImportFile').addEventListener('change', async (e) => {
  const file = e.target.files && e.target.files[0];
  e.target.value = ''; // reset so re-selecting the same file later still fires 'change'
  if (!file) return;
  ...
  try {
    ...
    showToast('Could not read that file — is it a valid JobMatch AI backup?');
    return;
  }
  if (!payload || typeof payload !== 'object' || !payload.data || typeof payload.data !== 'object') {
    showToast("That file doesn't look like a JobMatch AI backup.");
    return;
  }

  const ok = window.confirm(
    'Import this backup? It will overwrite your current profile, resumes, Q&A answers, ' +
    "saved/applied jobs, AI Settings, and Google Sheets sync settings on this browser with " +
    "the data from the file. This can't be undone."
  );
  if (!ok) return;

  let result;
  try {
    result = await sendMessage({ type: 'IMPORT_ALL_DATA', payload });
  } catch (err) {
    showToast(`Import failed: ${err.message}`);
    return;
  }
  showToast(`Imported ${result.importedKeys} section${result.importedKeys === 1 ? '' : 's'} — reloading…`);
...
// Applied jobs are still saved locally as before; when enabled here, each
// "Mark as Applied" also fires a POST to a user-deployed Apps Script web app
// (see the setup instructions delivered alongside this file) that appends a
// row to a Google Sheet. There's no Google API key involved — Sheets doesn't
// support writing to a private spreadsheet with a bare key, only OAuth or a
// script running with the sheet owner's own identity, which is what the
// Apps Script approach gives us for free.

document.getElementById('toggleSheetsSecretBtn').addEventListener('click', () => {
  const input = document.getElementById('sheetsSecret');
  const btn   = document.getElementById('toggleSheetsSecretBtn');
  if (input.type === 'password') {
    input.type      = 'text';
    btn.textContent = 'Hide';
  } else {
...
/**
 * Collects the Sheets Sync form fields and persists them via the background
 * service worker (SAVE_SHEETS_SYNC_SETTINGS message). Called from the save
 * button and pre-emptively before a test sync, mirroring saveSettings().
 */
async function saveSheetsSyncSettings() {
  const settings = {
    enabled:   document.getElementById('sheetsEnabled').checked,
    webAppUrl: document.getElementById('sheetsWebAppUrl').value.trim(),
    sheetName: document.getElementById('sheetsTabName').value.trim(),
    secret:    document.getElementById('sheetsSecret').value.trim(),
  };
  await sendMessage({ type: 'SAVE_SHEETS_SYNC_SETTINGS', settings });
  return settings;
}

document.getElementById('saveSheetsSettingsBtn').addEventListener('click', async () => {
  await saveSheetsSyncSettings();
  showToast('Sheets sync settings saved!');
});

/**
 * "Test Sync" button handler. Saves first (so the background uses the
 * latest URL/secret), then sends a TEST_SHEETS_SYNC message — the
 * background posts a { test: true } payload the Apps Script validates
 * without appending a row, so testing never writes a fake entry to the
 * sheet.
 */
document.getElementById('testSheetsSyncBtn').addEventListener('click', async () => {
  ...
  if (!settings.webAppUrl) {
    resultEl.textContent   = 'Enter a Web App URL first.';
    resultEl.className     = 'test-result error';
    resultEl.style.display = 'block';
    return;
  }
  ...
  try {
    resultEl.textContent   = 'Testing sync...';
    resultEl.className     = 'test-result loading';
    resultEl.style.display = 'block';

    await sendMessage({ type: 'TEST_SHEETS_SYNC' });
    resultEl.textContent = 'Sync successful! Your Apps Script is reachable and the secret matches.';
    resultEl.className   = 'test-result success';
  } catch (err) {
...
async function init() {
  try {
    ...
    if (settings) {
      ...
      document.getElementById('sProvider').value = settings.provider || 'openai';
      ...
      await updateProviderUI(settings.provider || 'openai', settings.model);
    ...
    if (sheetsSync) {
      document.getElementById('sheetsEnabled').checked = !!sheetsSync.enabled;
      document.getElementById('sheetsWebAppUrl').value = sheetsSync.webAppUrl || '';
...
// ─── Resume management ─────────────────────────────────────────────────────────
// Unlimited named resumes let the user maintain separate profiles for
// different types of job (e.g. engineering, management, consulting).
// The active resume's data is kept in sync with `profileData`; switching
// resumes saves the current profile via a deep-copy, then loads the target
// resume's data. Each resume also carries its own raw uploaded file
// (rawResumeBase64/resumeFileType) so "Generate Tailored Resume" always
// operates on the correct DOCX for whichever resume is active.

/**
 * All saved resumes. Persisted as 'resumes' in chrome.storage.local.
 * @type {Array<{id: string, name: string, profile: Object,
 *                rawResumeBase64: (string|null), resumeFileType: (string|null)}>}
 */
let resumes = [];

/**
 * Id of the currently active resume in `resumes`, or null if none exist yet.
 * Persisted as 'activeResumeId' in chrome.storage.local.
 * @type {string|null}
 */
let activeResumeId = null;

/**
 * Returns the currently active resume object, or null if none is active.
 * @returns {Object|null}
 */
...
/**
 * Returns the active resume, creating a fresh one first if none exists yet
 * (e.g. the very first resume a new user ever uploads or types into).
 * @returns {Object} The active resume — never null.
 */
function ensureActiveResume() {
  let active = getActiveResume();
  if (!active) {
    const id = `r_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    active = { id, name: `Resume ${resumes.length + 1}`, profile: profileData, rawResumeBase64: null, resumeFileType: null };
    resumes.push(active);
    activeResumeId = id;
  }
  return active;
...
/**
 * A blank profile object, used when creating a new resume or clearing the
 * form after the last resume is deleted.
 * @returns {Object}
 */
...
/** Restores the upload zone's default drag-and-drop prompt. */
 ...
 * Called before snapshot-copying the active resume, so the snapshot captures
 * any unsaved edits the user may have typed since the last explicit save.
 */
...
/**
 * Renders the resume switcher pills into #profileSlotBtns: one pill per
 * saved resume (click to switch, × to delete) plus a trailing "+ Add Resume"
 * button. Also refreshes the rename input to show the active resume's name.
 */
function renderResumeList() {
  const container = document.getElementById('profileSlotBtns');
  if (!container) return;
  container.innerHTML = '';

  resumes.forEach(r => {
    const wrap = document.createElement('div');
    wrap.className = 'profile-slot-btn-wrap';

    const btn = document.createElement('button');
    btn.className = 'btn btn-sm profile-slot-btn has-data' + (r.id === activeResumeId ? ' active' : '');
    btn.textContent = r.name || 'Resume';
    btn.title = r.name || 'Resume';
    btn.addEventListener('click', () => switchResume(r.id));
    wrap.appendChild(btn);

    const del = document.createElement('button');
    del.type = 'button';
    del.className = 'slot-delete-btn';
    del.title = `Delete ${r.name || 'this resume'}`;
    del.setAttribute('aria-label', `Delete ${r.name || 'this resume'}`);
    del.textContent = '×';
  ...
  addBtn.type = 'button';
  addBtn.className = 'btn btn-sm slot-add-btn';
  addBtn.textContent = '+ Add Resume';
...
/**
 * Persists `resumes` + `activeResumeId`, and mirrors the active resume's
 * profile/raw-file into the flat 'profile' / 'rawResumeBase64' /
 * 'resumeFileType' keys that background.js reads for AI calls and tailored
 * resume generation.
 */
async function persistResumes() {
  ...
  await chrome.storage.local.set({
    resumes,
    activeResumeId,
    profile: active ? active.profile : null,
    rawResumeBase64: active ? (active.rawResumeBase64 || null) : null,
    resumeFileType: active ? (active.resumeFileType || null) : null,
  });
}

/**
 * Loads `resumes` / `activeResumeId` from chrome.storage.local and refreshes
 * the UI. Falls back to converting the legacy `profileSlots` schema in
 * memory (and persisting the result) if the background migration hasn't
 * run yet — defensive, not the primary path. Silently ignores storage
 * errors (extension context loss, incognito mode, etc.).
 */
async function loadResumes() {
  try {
    const result = await chrome.storage.local.get([
      'resumes', 'activeResumeId', 'profileSlots', 'activeProfileSlot', 'slotNames'
    ]);
    if (Array.isArray(result.resumes)) {
      resumes = result.resumes;
      activeResumeId = result.activeResumeId || (resumes[0] && resumes[0].id) || null;
    } else if (Array.isArray(result.profileSlots)) {
      const slots = result.profileSlots;
      const names = result.slotNames || [];
      const activeIdx = result.activeProfileSlot || 0;
      const now = Date.now();
      resumes = [];
      activeResumeId = null;
      slots.forEach((profile, i) => {
        if (!profile) return;
        const id = `r_${now}_${i}`;
        resumes.push({ id, name: names[i] || `Resume ${i + 1}`, profile, rawResumeBase64: null, resumeFileType: null });
        if (i === activeIdx) activeResumeId = id;
      });
      if (!activeResumeId && resumes.length) activeResumeId = resumes[0].id;
    ...
    const active = getActiveResume();
    if (active) {
      profileData = JSON.parse(JSON.stringify(active.profile));
      populateProfileForm();
...
/**
 * Switches the active resume: snapshots any unsaved edits into the old
 * resume, loads the target resume's data into the form, and persists.
 * @param {string} id - The id of the resume to switch to.
 */
async function switchResume(id) {
  if (id === activeResumeId) return;

  // Capture any form edits and snapshot the current profile into the old resume
  syncCurrentProfileFromForm();
  const prevActive = getActiveResume();
  if (prevActive) prevActive.profile = JSON.parse(JSON.stringify(profileData));

  activeResumeId = id;
  const newActive = getActiveResume();

  profileData = newActive ? JSON.parse(JSON.stringify(newActive.profile)) : blankProfileData();
  populateProfileForm();
  if (newActive) {
    showResumeLoaded(profileData.resumeFileName || profileData.name || 'Resume');
  } else {
    resetUploadZone();
  }
  ...
  await persistResumes();
  renderResumeList();
  showToast(`Switched to ${newActive ? newActive.name : 'a new resume'}.`);
  renderAtsKeywordsByResume();
}

/**
 * Click handler for "+ Add Resume". Snapshots the current form into the
 * active resume, then creates and switches to a new blank one.
 */
async function addResume() {
  ...
  resumes.push({ id, name, profile: blankProfileData(), rawResumeBase64: null, resumeFileType: null });
  activeResumeId = id;

  profileData = blankProfileData();
  populateProfileForm();
  resetUploadZone();

  await persistResumes();
  renderResumeList();
  showToast(`Added "${name}". Upload a resume to get started.`);
  renderAtsKeywordsByResume();
}

/**
 * Deletes a resume after confirmation. If the deleted resume was active,
 * falls back to the first remaining resume, or the blank empty state if
 * none remain.
...
async function deleteResume(id) {
  ...
  if (activeResumeId === id) {
    activeResumeId = resumes.length ? resumes[0].id : null;
    const newActive = getActiveResume();
    profileData = newActive ? JSON.parse(JSON.stringify(newActive.profile)) : blankProfileData();
    populateProfileForm();
    if (newActive) {
  ...
  await persistResumes();
  renderResumeList();
  showToast(`Deleted "${resume.name}".`);
  renderAtsKeywordsByResume();
}

/**
 * "Rename" button handler for the resume name input.
 * Updates the active resume's name, persists it, and refreshes the pills.
 */
...
// ─── ATS keywords by resume ─────────────────────────────────────────────────────

/**
 * Renders a per-resume list of ATS keywords — the terms an Applicant
 * Tracking System scans a resume for — at the bottom of the Profile tab.
 * Derived entirely from that resume's own parsed data (skills,
 * certifications, and each project's technologies) via the shared
 * extractAtsKeywords() in lib/resumeKeywords.js — the exact same extraction
 * lib/resumeRanker.js uses to score resumes against a job description, so
 * this list and that local match score can never define "ATS keywords"
 * differently. Purely local: no storage read, just the in-memory `resumes`
 * array, so it updates instantly as the resume is edited.
 *
 * A term that appears in more than one place (e.g. a skill that's also
 * listed as a project technology) shows a ×N badge — a rough proxy for how
 * strongly that keyword is represented in the resume.
 */
function renderAtsKeywordsByResume() {
  const container = document.getElementById('atsKeywordsByResume');
  if (!container) return;

  if (!resumes.length) {
    container.innerHTML = '<p class="ats-resume-empty">Add a resume above to see its ATS keywords.</p>';
    return;
  }
  ...
  resumes.forEach(r => {
    ...
    if (sorted.length) {
      html += `<div class="skills-container">${sorted.map(([k, count]) =>
        `<span class="keyword-tag">${escapeHTML(k)}${count > 1 ? `<span class="freq">×${count}</span>` : ''}</span>`
      ).join('')}</div>`;
    } else {
      html += '<p class="ats-resume-empty">No skills, certifications, or project technologies added yet.</p>';
    }
    html += '</div>';
  });
