...
/**
 * Adds a job to the applied-jobs list, or retries the Google Sheets sync for
 * one already recorded.
 *
 * "Applied" (as shown by the side panel button) means the job's row was
 * confirmed appended to the user's Google Sheet — not merely that a local
 * record exists. So:
 *   - A URL never seen before creates a new local record and attempts the
 *     Sheets sync (if enabled).
 *   - A URL that already has a local record but never successfully synced
 *     (sync was disabled, or the webhook call failed) is treated as a retry:
 *     the SAME record is reused (its original `date` is preserved) and the
 *     sync is attempted again, updating `sheetsSynced`/`sheetsSyncError` in
 *     place. This lets the user click "Mark as Applied" again after fixing
 *     their Sheets Sync settings without creating a duplicate local entry.
 *   - A URL that already synced successfully is a true duplicate — returned
 *     as-is (with `duplicate: true`) without re-appending a row to the sheet.
 *
...
async function handleMarkApplied(jobData) {
  ...
  // Match by *normalized* URL so the same posting opened from different
  // sources (UTM tags, click IDs, etc.) is treated as the same job.
  const incoming = normalizeUrlForCache(jobData.url || '');
  const existingIdx = jobs.findIndex(j => normalizeUrlForCache(j.url || '') === incoming);
  const existing = existingIdx !== -1 ? jobs[existingIdx] : null;
  ...
  const job = existing || {
    ...
    url: jobData.url || '',
    resume: jobData.resume || '', // Which saved resume this job was analyzed/applied against
    date: new Date().toISOString().split('T')[0]
    // Note: analysis is intentionally omitted here to keep the applied list leaner
  };

  // Attempt (or retry) the Google Sheets sync — *before* the local save so
  // the outcome can be stamped onto the stored record in the same write.
  // Never blocks or skips the local save on failure: the applied job is
  // always recorded locally either way, sync is what determines whether the
  // UI shows "Applied" or leaves the button retryable.
  const sheetsSettings = await getSheetsSyncSettings();
  if (sheetsSettings.enabled && sheetsSettings.webAppUrl) {
    try {
      await postToSheetsWebApp({ job });
      job.sheetsSynced = true;
      delete job.sheetsSyncError;
    } catch (e) {
      job.sheetsSynced = false;
      job.sheetsSyncError = e.message;
    }
  } else {
    job.sheetsSynced = false;
    job.sheetsSyncError = 'Google Sheets sync is not enabled. Go to Profile → Settings.';
  }
  ...
  if (existing) {
    ...
    jobs.unshift(job);
    // Cap at 500 entries — applied list is larger than saved list since
    // users typically apply to many more jobs than they bookmark.
...
// ─── Google Sheets Sync ─────────────────────────────────────────────────────
// Optional: pushes each newly-applied job to a Google Sheet via a small
// Apps Script web app the user deploys once (see the setup instructions
// delivered alongside this feature). A bare API key can't do this — writing
// to a private spreadsheet needs either OAuth consent or a script running
// with the sheet owner's own identity, and the latter is far simpler to
// wire into a browser extension than a full OAuth flow. Fails soft: a sync
// failure never blocks or undoes the local appliedJobs save above.

const SHEETS_SYNC_FETCH_TIMEOUT = 8000; // ms — generous; Apps Script cold starts can be slow

/**
 * Retrieves the user's Google Sheets Sync configuration from local storage.
 * @async
 * @returns {Promise<{enabled: boolean, webAppUrl: string, secret: string}>}
 */
async function getSheetsSyncSettings() {
  const result = await chrome.storage.local.get('sheetsSync');
  return result.sheetsSync || { enabled: false, webAppUrl: '', sheetName: '', secret: '' };
}
...
/**
 * Snapshots every BACKUP_KEYS entry currently in storage into one portable
 * object, ready to be JSON.stringify'd and downloaded by the caller.
 *
 * @async
 * @returns {Promise<Object>} `{ app, exportType, version, exportedAt, data }`
 *   — `data` holds whatever subset of BACKUP_KEYS is actually populated
 *   (an unused key like `sheetsSync` before it's ever configured is simply
 *   absent, not written as null/undefined).
 */
...
/**
 * Restores a previously-exported backup, overwriting this browser's current
 * data for every key the file contains.
 *
 * Only ever writes keys from BACKUP_KEYS — a malformed, tampered, or
 * future/older-version file can't inject arbitrary storage keys this way.
 * A key BACKUP_KEYS knows about but the file doesn't include (e.g. an older
 * export made before Sheets sync existed) is left untouched rather than
 * cleared, so restoring a partial/older backup can't silently wipe fields
 * it never had.
 *
 * @async
 * @param {Object} payload - The parsed JSON file contents; expected shape
 *   `{ data: { ...BACKUP_KEYS subset } }` as produced by exportAllData().
 * @returns {Promise<{success: true, importedKeys: number}>}
 * @throws {Error} If `payload` isn't a recognizable backup, or contains
 *   none of the known keys.
 */
async function importAllData(payload) {
  if (!payload || typeof payload !== 'object' || !payload.data || typeof payload.data !== 'object') {
    throw new Error("That file doesn't look like a JobMatch AI backup.");
  }
  const toWrite = {};
  let importedKeys = 0;
  for (const key of BACKUP_KEYS) {
    if (Object.prototype.hasOwnProperty.call(payload.data, key)) {
      toWrite[key] = payload.data[key];
      importedKeys++;
    }
  }
  if (importedKeys === 0) {
    throw new Error('That backup file has nothing importable in it.');
  }
...
/**
 * POSTs a JSON payload to the configured Apps Script web app.
 *
 * Sent with a `text/plain` Content-Type rather than `application/json` —
 * Apps Script web apps don't handle the CORS preflight (OPTIONS) request a
 * real `application/json` fetch would trigger, so this keeps the request a
 * CORS "simple request" while the Apps Script side still JSON.parses the
 * body regardless of the declared content type.
 *
 * @async
 * @param {Object} extra - Merged into the request body alongside the secret,
 *   e.g. `{ job: {...} }` or `{ test: true }`.
 * @returns {Promise<Object>} The Apps Script's parsed JSON response.
 * @throws {Error} If no Web App URL is configured, on network failure or
 *   timeout, on a non-2xx response, or if the script itself reports
 *   `success: false` (e.g. secret mismatch).
 */
async function postToSheetsWebApp(extra) {
  const settings = await getSheetsSyncSettings();
  if (!settings.webAppUrl) throw new Error('No Sheets Web App URL configured. Go to Profile → Settings.');

  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), SHEETS_SYNC_FETCH_TIMEOUT);
  try {
    const res = await fetch(settings.webAppUrl, {
      method: 'POST',
      signal: controller.signal,
      headers: { 'Content-Type': 'text/plain;charset=utf-8' },
      body: JSON.stringify({ secret: settings.secret || '', sheetName: settings.sheetName || '', ...extra }),
    });
    if (!res.ok) throw new Error(`Apps Script returned HTTP ${res.status}`);
    const data = await res.json().catch(() => null);
    if (!data || data.success !== true) {
      throw new Error((data && data.error) || 'Apps Script did not confirm success — check the Web App URL and secret.');
    }
    return data;
  } catch (e) {
    if (e.name === 'AbortError') throw new Error('Sheets sync timed out.');
    throw e;
  } finally {
...
/**
 * Verifies the configured Apps Script web app is reachable and the shared
 * secret matches, without appending a row — used by the Settings page's
 * "Test Sync" button.
 * @async
 * @throws {Error} Same conditions as postToSheetsWebApp.
 * @returns {Promise<Object>} The Apps Script's parsed JSON response.
 */
async function handleTestSheetsSync() {
...
const handlers = {
  ...
  'GET_SHEETS_SYNC_SETTINGS': (msg) => getSheetsSyncSettings(),
  ...
  'EXPORT_ALL_DATA': (msg) => exportAllData(),
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === 'install') {
    ...
    chrome.storage.local.set({
      ...
      profile: null,          // No resume uploaded yet — mirrors the active resume's profile
      ...
      sheetsSync: { enabled: false, webAppUrl: '', secret: '' },
    });
  } else if (details.reason === 'update') {
    migrateProfileSlotsToResumes();
  }
});

/**
 * One-time migration from the old fixed-3-slot resume schema
 * (profileSlots / activeProfileSlot / slotNames, plus a single global
 * rawResumeBase64 key) to the new unlimited `resumes` array keyed by a
 * stable id. Runs on every extension update but is a no-op once `resumes`
 * already exists.
 *
 * Only the previously-active slot's raw DOCX survives the migration, since
 * the old schema only ever stored one raw file globally regardless of which
 * slot was active when it was uploaded — that data loss predates this
 * migration, it isn't introduced by it.
 */
async function migrateProfileSlotsToResumes() {
  try {
    const old = await chrome.storage.local.get([
      'resumes', 'profileSlots', 'activeProfileSlot', 'slotNames',
      'rawResumeBase64', 'resumeFileType'
    ]);
    if (old.resumes || !old.profileSlots) return; // already migrated, or nothing to migrate

    const slots = old.profileSlots || [];
    const names = old.slotNames || [];
    const activeIdx = old.activeProfileSlot || 0;
    const now = Date.now();
    ...
    slots.forEach((profile, i) => {
      ...
      resumes.push({
        id,
        name: names[i] || `Resume ${i + 1}`,
        profile,
        rawResumeBase64: i === activeIdx ? (old.rawResumeBase64 || null) : null,
        resumeFileType: i === activeIdx ? (old.resumeFileType || null) : null,
      });
    ...
    await chrome.storage.local.set({
      resumes,
      activeResumeId,
      profile: activeResume ? activeResume.profile : null,
    });
  } catch (e) {
    // Storage errors here shouldn't block extension startup — the old keys
    // remain untouched and profile.js's defensive fallback covers this case.
  }
}
