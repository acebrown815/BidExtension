...
(function () {
  ...
  function normalizeUrlForCache(rawUrl) {
    ...
    // Ashby splits one posting into two pages that share a single job id —
    // jobs.ashbyhq.com/<company>/<jobId> (the overview) and
    // .../<jobId>/application (the form) — often opened as two separate
    // tabs/links rather than a same-tab client-side route change, so
    // content.js's tab-scoped JD/analysis cache never bridges them. Without
    // this, the two pages normalize to different cache keys and get treated
    // as different jobs (re-analyzed, and separately logged as applied).
    // Stripping the "/application" suffix collapses both to the same key.
    if (/(^|\.)ashbyhq\.com$/i.test(u.hostname) && u.pathname.endsWith('/application')) {
