 ...
 *
 * Revised languages AND skillCategories items are ADDED to the skills used
 * for scoring rather than replacing the flat skills list outright —
 * profile.skills mixes every category (languages, frameworks, cloud, ...)
 * with no structure telling us which entries came from which category, so
 * there's no clean way to subtract from it. Since the score only cares
 * about which skills are present, adding is enough; nothing needs removing
 * here (the DOCX-level lines are trimmed separately, for readability, not
 * for scoring).
 *
 * This is what actually makes the tailored resume's displayed Match Score
 * reflect the tailoring: without folding skillCategories in here too, the
 * score stayed identical to the pre-tailoring score no matter how much the
 * skills section itself changed in the DOCX — the rescore call was
 * comparing the job description against the OLD flat skills list the
 * whole time, blind to every skill the skills-section rewrite had just
 * added (e.g. "Java", "Spring Boot", "JMeter" for a Java-focused JD).
 *
 ...
 * @param {Array<{label: string, items: string[]}>} [skillCategories] - AI-revised non-Languages skill categories.
 * @returns {Object} A shallow-cloned profile with summary/skills/experience updated for scoring.
 */
export function buildTailoredProfileForRescoring(profile, newSummary, languages, rewrittenBullets, skillCategories) {
  ...
  const categoryItems = Array.isArray(skillCategories)
    ? skillCategories.flatMap(c => Array.isArray(c && c.items) ? c.items : []).filter(Boolean)
    : [];
  ...
  const addedSkills = [];
  const addedSkillsLower = new Set();
  [...langs, ...categoryItems].forEach(s => {
    const lower = String(s).toLowerCase().trim();
    if (!lower || originalSkillsLower.has(lower) || addedSkillsLower.has(lower)) return;
    addedSkillsLower.add(lower);
    addedSkills.push(s);
  });
  ...
  return {
    ...
    skills: addedSkills.length > 0 ? [...originalSkills, ...addedSkills] : originalSkills,
